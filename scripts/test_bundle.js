        ...
        function drawVectorSignalsAndTargets(timeNow) {
            ...
            if (mouseCssX >= 0 && mouseCssY >= 0 && mouseCssX <= cssW && mouseCssY <= cssH) {
                for (const sig of calculatedSignals) {
                    ...
                    // C. Odaktayken (progress > 0.25) genişleyen mum veya ortalanmış TP/SL etiketleri üzerinde mi?
                    ...
                    if (focusAnim.currentSigIndex === sig.index && focusAnim.progress > 0.25) {
                        ...
                        const isOverEntry = (Math.abs(mouseCssX - screenX) <= 90) && (Math.abs(mouseCssY - entryY) <= 14);
                        const isOverEnlargedTag = (Math.abs(mouseCssX - screenX) <= 65) && (Math.abs(mouseCssY - (isBuy ? lowY + 36 : highY - 36)) <= 18);

                        isOverEnlargedFocusZone = isOverEnlargedCandle || isOverTp || isOverSl || isOverEntry || isOverEnlargedTag;
                    }
