        ...
        // ============================================================
        // 🎆 HAVAİ FİŞEK VE PARÇACIK ANİMASYON MOTORU
        // ============================================================
        const fireworkColors = ['#10b981', '#38bdf8', '#fbbf24', '#f43f5e', '#a855f7', '#ec4899', '#00f2fe', '#ffffff'];

        function drawFireworkExplosion(ctx, x, y, timeNow, seed) {
            const particleCount = 14;
            const cycle = (timeNow * 0.002 + seed * 1.3) % 1.0;
            const radius = cycle * 34;
            const alpha = Math.max(0, 1.0 - cycle * 1.1);

            ctx.save();
            for (let i = 0; i < particleCount; i++) {
                const angle = (i / particleCount) * Math.PI * 2 + seed + (cycle * 0.8);
                const sparkDist = radius * (0.6 + 0.4 * Math.sin(i * 3 + seed));
                const px = Math.round(x + Math.cos(angle) * sparkDist);
                const py = Math.round(y + Math.sin(angle) * sparkDist);
                const color = fireworkColors[(i + Math.floor(seed * 10)) % fireworkColors.length];
            ...
            ctx.shadowBlur = 14;
            ctx.globalAlpha = Math.max(0, 1.0 - cycle * 2.0);
            ctx.beginPath();
        ...
        // ============================================================
        // HiDPI DİNAMİK VEKTÖR TP / SL VE ROZET ÇİZİCİSİ (SİNEMATİK SPOT IŞIKLI)
        // ============================================================
        function drawVectorSignalsAndTargets(timeNow) {
            ...
            // 1. Hovered Sinyal Kontrolü
            const hoveredSig = (hoveredCandleIdx >= 0) ? calculatedSignals.find(s => Math.abs(s.index - hoveredCandleIdx) <= 0.5) : null;

            // 2. Eğer sinyal üzerine gelinmişse arka planı sinematik olarak karart (Spotlight Vignette)
            if (hoveredSig) {
                ctx2d.save();
                ctx2d.fillStyle = 'rgba(3, 7, 18, 0.72)';
                ctx2d.fillRect(0, 0, cssW, cssH);
                ctx2d.restore();
            }

            // 3. Normal / Arka Plan Sinyalleri Çizimi
            for (const sig of calculatedSignals) {
                if (sig.index < viewStart - 8 || sig.index > viewEnd + 8) continue;
                if (hoveredSig && sig === hoveredSig) continue; // Hovered olanı en üstte devasa çizeceğiz

                ctx2d.save();
                if (hoveredSig) {
                    ctx2d.globalAlpha = 0.18; // Diğer tüm sinyalleri iyice karart
                }
                ...
                if (isBacktestActive && isTpWin && !hoveredSig) {
                    drawFireworkExplosion(ctx2d, screenX, badgeY, timeNow, sig.index * 0.17);
                }
            ...
            // 4. SPOTLIGHT MODU: HOVERED SİNYALİ BÜYÜTEREK VE PARLATARAK ÖNE ÇIKAR
            if (hoveredSig) {
                const sig = hoveredSig;
                const candleCenter = sig.index + 0.5;
                const screenX = Math.round(((candleCenter - viewStart) / visibleCount) * cssW);
                const candle = candleDataBase[sig.index];
                ...
                if (candle) {
                    ...
                    // A. DİKEY PARLAK IŞIK SÜTUNU (SPOTLIGHT BEAM)
                    const beamGrad = ctx2d.createLinearGradient(screenX, 0, screenX, cssH);
                    beamGrad.addColorStop(0, 'rgba(56, 189, 248, 0.0)');
                    beamGrad.addColorStop(0.3, isBuy ? 'rgba(16, 185, 129, 0.12)' : 'rgba(239, 68, 68, 0.12)');
                    beamGrad.addColorStop(0.5, isBuy ? 'rgba(16, 185, 129, 0.22)' : 'rgba(239, 68, 68, 0.22)');
                    beamGrad.addColorStop(0.7, isBuy ? 'rgba(16, 185, 129, 0.12)' : 'rgba(239, 68, 68, 0.12)');
                    beamGrad.addColorStop(1, 'rgba(56, 189, 248, 0.0)');
                    
                    const beamWidth = Math.max(48, (cssW / visibleCount) * 3.5);
                    ctx2d.fillStyle = beamGrad;
                    ctx2d.fillRect(screenX - beamWidth / 2, 0, beamWidth, cssH);

                    // B. TP VE SL HEDEF ALANLARI (DOLGULU RİSK/KÂR KUTUSU)
                    const projDistance = isBacktestActive ? Math.min(sig.exitIndex - sig.index, 35) : 16;
                    const projEndX = Math.round(Math.min(cssW - 20, screenX + (cssW / visibleCount) * Math.max(12, projDistance)));

                    // TP Alanı (Yeşil Vurgu Kutusu)
                    const tpTop = Math.min(entryY, tpY);
                    const tpHeight = Math.max(2, Math.abs(tpY - entryY));
                    ctx2d.fillStyle = 'rgba(16, 185, 129, 0.22)';
                    ctx2d.fillRect(screenX, tpTop, projEndX - screenX, tpHeight);

                    // SL Alanı (Kırmızı Vurgu Kutusu)
                    const slTop = Math.min(entryY, slY);
                    const slHeight = Math.max(2, Math.abs(slY - entryY));
                    ctx2d.fillStyle = 'rgba(239, 68, 68, 0.22)';
                    ctx2d.fillRect(screenX, slTop, projEndX - screenX, slHeight);

                    // C. KALIN IŞILDAYAN TP VE SL ÇİZGİLERİ
                    // TP Çizgisi
                    ...
                    // Giriş Seviyesi (Nötr Sarı Çizgi)
                    ...
                    ctx2d.beginPath();
                    ctx2d.moveTo(screenX, entryY);
                    ctx2d.lineTo(projEndX, entryY);
                    ctx2d.stroke();
                    ctx2d.setLineDash([]);

                    // D. BÜYÜTÜLMÜŞ SPOTLIGHT SİNYAL MUMU (2X BOYUT VE NEON GÖVDE)
                    const candleW = Math.max(14, (cssW / visibleCount) * 1.85);
                    ...
                    // E. DEVASA HOLOGRAFİK TP / SL ROZETLERİ
                    ...
                    // 1. TP Holografik Rozeti
                    ctx2d.fillStyle = 'rgba(6, 78, 59, 0.95)';
                    ctx2d.strokeStyle = '#10b981';
                    ctx2d.lineWidth = 2;
                    ctx2d.shadowColor = '#10b981';
                    ctx2d.shadowBlur = 16;
                    ...
                    const tpW = Math.round(tpMetrics.width) + 16;
                    const tpH = 24;
                    ctx2d.beginPath();
                    ctx2d.roundRect(projEndX - tpW / 2, tpY - tpH / 2, tpW, tpH, 6);
                    ctx2d.fill();
                    ctx2d.stroke();
                    ctx2d.fillStyle = '#ffffff';
                    ctx2d.textAlign = 'center';
                    ctx2d.textBaseline = 'middle';
                    ctx2d.fillText(tpText, projEndX, tpY);

                    // 2. SL Holografik Rozeti
                    ctx2d.fillStyle = 'rgba(127, 29, 29, 0.95)';
                    ctx2d.strokeStyle = '#ef4444';
                    ctx2d.lineWidth = 2;
                    ctx2d.shadowColor = '#ef4444';
                    ctx2d.shadowBlur = 16;
                    ...
                    const slW = Math.round(slMetrics.width) + 16;
                    const slH = 24;
                    ctx2d.beginPath();
                    ctx2d.roundRect(projEndX - slW / 2, slY - slH / 2, slW, slH, 6);
                    ctx2d.fill();
                    ctx2d.stroke();
                    ctx2d.fillStyle = '#ffffff';
                    ctx2d.textAlign = 'center';
                    ctx2d.textBaseline = 'middle';
                    ctx2d.fillText(slText, projEndX, slY);

                    // 3. Giriş Bilgi Rozeti
                    ...
                    const entryW = Math.round(entryMetrics.width) + 14;
                    const entryH = 20;
                    ctx2d.beginPath();
                    ctx2d.roundRect(screenX + 10, entryY - entryH / 2, entryW, entryH, 5);
                    ctx2d.fill();
                    ctx2d.stroke();
                    ctx2d.fillStyle = '#fef08a';
                    ctx2d.textAlign = 'center';
                    ctx2d.textBaseline = 'middle';
                    ctx2d.fillText(entryText, screenX + 10 + entryW / 2, entryY);

                    // F. BÜYÜTÜLMÜŞ SİNYAL ETİKET ROZETİ
                    const badgeY = isBuy ? lowY + 34 : highY - 34;
