        ...
        // ============================================================
        // HiDPI DİNAMİK VEKTÖR TP / SL VE ROZET ÇİZİCİSİ
        // ============================================================
        function drawVectorSignalsAndTargets(timeNow) {
            ...
            for (const sig of calculatedSignals) {
                ...
                // 1. KAZANAN SİNYALDE HAVAİ FİŞEK PATLAMASI
                if (isBacktestActive && isTpWin) {
                ...
                // 3. STOP OLAN SİNYALDE ÇARPI (X) İŞARETİ
                ...
                if (visibleCount < 400) {
                    ...
                    // TP Noktası & Etiketi
                    ctx2d.setLineDash([]);
                    ctx2d.fillStyle = isTpWin ? '#10b981' : (isSlLoss ? '#64748b' : '#10b981');
                    ctx2d.shadowColor = ctx2d.fillStyle;
                    ctx2d.shadowBlur = isTpWin ? 10 : 6;
                    ctx2d.beginPath();
                    ctx2d.arc(projEndX, tpY, isTpWin ? 4.5 : 3.5, 0, Math.PI * 2);
                    ctx2d.fill();

                    ctx2d.font = 'bold 10.5px "SF Pro Text", "Segoe UI", sans-serif';
                    ctx2d.textAlign = 'left';
                    ctx2d.textBaseline = 'middle';
                    const winBadge = isTpWin ? ` ✅ +${sig.pnlPct.toFixed(1)}%` : '';
                    ctx2d.fillText(`🎯 TP: ${sig.tp.toFixed(1)}${winBadge}`, projEndX + 6, tpY);
                    ...
                    // SL Noktası & Etiketi
                    ctx2d.setLineDash([]);
                    ctx2d.fillStyle = isSlLoss ? '#ef4444' : (isTpWin ? '#64748b' : '#ef4444');
                    ctx2d.shadowColor = ctx2d.fillStyle;
                    ctx2d.shadowBlur = isSlLoss ? 10 : 6;
                    ctx2d.beginPath();
                    ctx2d.arc(projEndX, slY, isSlLoss ? 4.5 : 3.5, 0, Math.PI * 2);
                    ctx2d.fill();
                    ...
                    ctx2d.textAlign = 'left';
                    ctx2d.textBaseline = 'middle';
                    const lossBadge = isSlLoss ? ` ❌ -${Math.abs(sig.pnlPct).toFixed(1)}%` : '';
                    ctx2d.fillText(`🛡️ SL: ${sig.sl.toFixed(1)}${lossBadge}`, projEndX + 6, slY);
