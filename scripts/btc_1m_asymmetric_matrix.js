// ==============================================================================
// 🔬 1M ASİMETRİK VE DİNAMİK MOMENTUM OPTİMİZASYONU
// ==============================================================================
import fs from 'fs';
...
async function runAsymmetric() {
    ...
    // Her sinyal tipi için ayrı ayrı en karlı TP/SL mesafesini bulalım
    for (const targetType of ['BUY1', 'BUY2', 'SELL1', 'SELL2']) {
        const filteredSigs = ind.rawSignals.filter(s => s.type === targetType);
        let best = { ret: -999, winRate: 0, config: null };

        for (let slAtr of [0.6, 0.8, 1.0, 1.2, 1.5, 2.0]) {
            for (let tpAtr of [0.8, 1.0, 1.2, 1.5, 2.0, 2.5, 3.0]) {
                for (let beTrig of [0, 0.5, 0.7, 0.9]) {
                    ...
                    if (ret > best.ret || (Math.abs(ret - best.ret) < 0.1 && winRate > best.winRate)) {
                        best = { ret, winRate, total, tpCount, slCount, config: { slAtr, tpAtr, beTrig } };
                    }
                }
            }
        }

        console.log(`\n🎯 [${targetType}] (Toplam ${filteredSigs.length} sinyal) EN İYİ PROFİLİ:`);
        console.log(`   ⚙️ Parametre: SL: ${best.config.slAtr}x ATR | TP: ${best.config.tpAtr}x ATR | BE: ${best.config.beTrig}x`);
        console.log(`   🏆 Kazanma Oranı: %${best.winRate.toFixed(1)} (${best.tpCount} TP / ${best.slCount} SL)`);
        console.log(`   💰 Sadece Bu Sinyallerle Kâr: +%${best.ret.toFixed(2)}`);
    }
}

runAsymmetric().catch(console.error);
