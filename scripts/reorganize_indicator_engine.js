...
// First remove any misplaced duplicate definitions
content = content.replace(/\/\/ ============================================================\s*\/\/ 📊 TEKNİK HESAPLAMA MOTORU VE FORMÜL KÜTÜPHANESİ[\s\S]*?const BUILTIN_INDICATOR_DEFS = {/, 'const BUILTIN_INDICATOR_DEFS = {');
content = content.replace(/\/\/ ============================================================\s*\/\/ ⚡ MODÜLER GÖSTERGE MOTORU VE SCRIPT ENGINE RUNTIME[\s\S]*?function renderOverlay\(now\) {/m, 'function renderOverlay(now) {');

// Clean Complete Indicator Engine Code
const completeIndicatorEngine = `
        // ============================================================
        ...
        window.renderFxModalList = function(tab, query = '') {
...
// Insert completeIndicatorEngine at the start of <script>
content = content.replace(
    '        // WebGL 2.0 & Global Değişkenler',
    completeIndicatorEngine + '\n        // WebGL 2.0 & Global Değişkenler'
);

fs.writeFileSync('indikator_sablonu.html', content, 'utf8');
console.log('Successfully reorganized complete indicator engine and script runtime at top of script!');
